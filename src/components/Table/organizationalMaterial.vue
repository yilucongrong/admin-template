  ...
  <div class="app-container calendar-list-container popup">
    <div class="filter-container">
      <div class="filter-items">
        <dictItemSelect class="filter-item" :placeholder="'物料类型'"
                        :dictCode="DICT_CODE.materielType" v-model="listQuery.materielType">
        </dictItemSelect>
        <dictItemSelect class="filter-item" :placeholder="'物料分类'"
                        :dictCode="DICT_CODE.materielClassify" v-model="listQuery.materielClassify">
        </dictItemSelect>
        <dictItemSelect class="filter-item" :placeholder="'物料组'"
                        :dictCode="DICT_CODE.materielGroup" v-model="listQuery.materielGroup">
        </dictItemSelect>
        <el-input @keyup.enter.native="handleQuery" class="filter-item" :placeholder="'规格、型号'"
                  size="mini" v-model="listQuery.specification">
        </el-input>

        <el-input @keyup.enter.native="handleQuery" class="filter-item" :placeholder="'物料编码'"
                  size="mini" v-model="listQuery.materielCode">
        </el-input>
        <el-input @keyup.enter.native="handleQuery" class="filter-item" :placeholder="'物料名称'"
                  size="mini" v-model="listQuery.materielName">
        </el-input>
        <el-input @keyup.enter.native="handleQuery" class="filter-item" :placeholder="'材质'"
                  size="mini" v-model="listQuery.materialQuality">
        </el-input>
        <dictItemSelect class="filter-item" :placeholder="'状态'"
                        :dictCode="DICT_CODE.productionState" v-model="listQuery.productionState">
        </dictItemSelect>

        <el-button class="filter-item" size="mini" type="primary" icon="el-icon-search" @click="handleQuery">{{$t('table.search')}}</el-button>
      </div>
    </div>
    <div class="table-container">
      <el-table :key='tableKey' :data="list" border fit highlight-current-row
                ...
                height="315" header-cell-class-name="header-cell">
        ...
        <el-table-column show-overflow-tooltip width="120" align="center" label="SNP" prop="stockContain"></el-table-column>
