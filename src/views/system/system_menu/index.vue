      ...
      <el-col :span="21">
        <div class="app-container">
          <div class="filter-container">
            <div class="main-header">
                <div class="select-content">
                  <el-input size="small" :placeholder="$t('systemMenu.catalogCode')" v-model="listQuery.catalogCode" class="filter-item" @keyup.enter.native="handleFilter"/>
                  <el-input size="small" :placeholder="$t('systemMenu.catalogName')" v-model="listQuery.catalogName" class="filter-item" @keyup.enter.native="handleFilter"/>
            ...
            <div class="main-body">
              <div class="btn">
                ...
                <el-button size="small" class="filter-item" type="primary" icon="el-icon-edit-outline" @click="handleAdd">{{$t('systemMenu.resourceAssociation')}}</el-button>
                <el-button size="small" class="filter-item" type="primary" icon="el-icon-edit-outline" @click="handleBtnAuthority">{{$t('systemMenu.buttonOprate')}}</el-button>
              </div>
              <el-table
                ...
                <el-table-column show-overflow-tooltip :label="$t('systemMenu.catalogName')" width="80" align="center">
                  <template slot-scope="scope">
                    <span>{{ scope.row.catalogName }}</span>
                  </template>
                </el-table-column>
                <el-table-column show-overflow-tooltip :label="$t('systemMenu.catalogEngName')" min-width="80">
                  <template slot-scope="scope">
                    <span>{{ scope.row.catalogEngName }}</span>
                  </template>
                </el-table-column>
                <el-table-column show-overflow-tooltip :label="$t('systemMenu.catalogOrder')" width="60" align="center">
                  <template slot-scope="scope">
                    <span>{{ scope.row.catalogOrder }}</span>
                  </template>
                </el-table-column>
                <el-table-column show-overflow-tooltip :label="$t('systemMenu.nodeType')" width="120" align="center">
                  <template slot-scope="scope">
                   <span>{{ scope.row.nodeType|nodeFilter }}</span>
                  </template>
                </el-table-column>
                <el-table-column show-overflow-tooltip :label="$t('systemMenu.parentId')" width="120">
                  <template slot-scope="scope">
                    <span>{{ pidFilter(scope.row.parentId) }}</span>
                  </template>
                </el-table-column>
                <el-table-column show-overflow-tooltip :label="$t('systemMenu.url')" width="120">
                  <template slot-scope="scope">
                    <span>{{ scope.row.url }}</span>
                  </template>
                </el-table-column>
                <el-table-column show-overflow-tooltip :label="$t('systemMenu.remark')" align="center" width="180">
                  <template slot-scope="scope">
          ...
          <el-dialog custom-class="dialog-custom" :title="dialogStatus=='create'?$t('table.add'):$t('table.edit')" :visible.sync="dialogFormVisible" v-dialogDrag :close-on-click-modal="false">
            <el-form class="small-space" ref="dataForm" :inline="true" :rules="rules" :model="temp" label-width="90px">
              
                <el-form-item :label="$t('systemMenu.catalogCode')" prop="catalogCode">
                  <el-input v-if="dialogStatus=='update'" v-model="temp.catalogCode" disabled="disabled"/>
                ...
                <el-form-item :label="$t('systemMenu.parentId')" prop="parentId">
                    <select-tree :options="treeData" v-model="temp.parentId"  :props="defaultProps" :value="0" />
                ...
                <el-form-item label="设备端口" prop="allowEmptyValue">
           ...
           <!-- 资源关联弹窗 -->
          <el-dialog :title="$t('systemMenu.resourceAssociation')" :visible.sync="dialogFormUser" v-dialogDrag :close-on-click-modal="false">
            
            <el-tabs v-model="activeName2" type="border-card" @tab-click="handleTabClick">
              <el-tab-pane :label="$t('table.tbSelected')" name="first">
                  <div class="main-header">
                      <div class="select-content">
                        <el-button size="small" class="filter-item" type="primary" icon="el-icon-select" @click="authority">{{ $t('table.empower') }}</el-button> 
                      </div>
                  </div>
                  <div>
                      <el-table v-loading="listLoading" :key="tableKey" :data="listUnrelation" border fit height="350px" highlight-current-row
                          style="width: 100%;"
                          @selection-change='selectRowUser'>
                        <el-table-column type="selection" width="30">
                        </el-table-column>
                        <el-table-column show-overflow-tooltip :label="$t('systemResource.endpointCode')" width="120" align="center">
                          <template slot-scope="scope">
                            <span>{{ scope.row.endpointCode }}</span>
                          </template>
                        </el-table-column>
                        <el-table-column show-overflow-tooltip :label="$t('systemResource.endpointName')" width="120" >
                          <template slot-scope="scope">
                            <span>{{ scope.row.endpointName }}</span>
                          </template>
                        </el-table-column>
                        <el-table-column show-overflow-tooltip :label="$t('systemResource.method')" width="120" align="center">
                          <template slot-scope="scope">
                            <span>{{ scope.row.method }}</span>
                          </template>
                        </el-table-column>
                        <el-table-column show-overflow-tooltip :label="$t('systemResource.url')" width="120" align="center">
                          <template slot-scope="scope">
                            <span>{{ scope.row.url }}</span>
                          </template>
                        </el-table-column>
                        <el-table-column show-overflow-tooltip :label="$t('systemResource.remark')" width="120" align="center">
                          <template slot-scope="scope">
                            <span>{{ scope.row.remark }}</span>
                          </template>
                        </el-table-column>
              ...
              <el-tab-pane :label="$t('table.selected')" name="second">
                  <div>
                      <div class="select-content">
                          <el-button size="small" class="filter-item" type="primary" icon="el-icon-select" @click="unauthority">{{$t('table.unauthority')}}</el-button> 
                      </div>
                      <el-table v-loading="listLoading" :key="tableKey" :data="listRelation" border fit height="350px" highlight-current-row
                          style="width: 100%;"
                          @selection-change='selectRowUser'>
                        <el-table-column type="selection" width="30">
                        </el-table-column>
          ...
          <!-- 按钮权限弹框 -->
          <el-dialog custom-class="dialog-custom" :title="'按钮操作'" :visible.sync="btnAuthorityDialog" v-dialogDrag :close-on-click-modal="false">
            <div class="table_tag">
                <el-button class="filter-item" size="small" type="primary" @click="addButton" icon="el-icon-plus">{{ $t('table.add') }}</el-button>
                <el-button class="filter-item" size="small" type="primary" @click="deleteButton" icon="el-icon-plus">{{ $t('table.delete') }}</el-button>
            </div>
            <el-table
                :data="btnFucList"
                border
                ...
                style="width: 100%;"
                ref="detailtable"
                @selection-change='btnSelectChange' cell-class-name="table-cell" header-cell-class-name="header-cell">
                <el-table-column type="selection" fixed width="30" align="center"></el-table-column>
                <el-table-column show-overflow-tooltip :label="'行号'" prop="rowId">
                    <template slot-scope="scope">
                        <el-input v-model="scope.row.rowId" type="text" required></el-input>         
                    </template>
                </el-table-column>
                <el-table-column show-overflow-tooltip :label="'功能'" prop="btnFunc">
                    <template slot-scope="scope">
                        <el-input v-model="scope.row.btnFunc" type="text" required></el-input>         
                    </template>
                </el-table-column>
                <el-table-column show-overflow-tooltip :label="'按钮名称'" prop="btnName">
                    <template slot-scope="scope">
                        <el-input v-model="scope.row.btnName" type="text" required></el-input>         
                    </template>
                </el-table-column>
            </el-table>
            ...
            <div slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" @click="confirmBtnList">{{$t('table.confirm')}}</el-button>
            </div>
          </el-dialog>
        </div>
      </el-col>
...
import { selectDatas, addData, selectData, deleteData,updataData,selectEndpoints,addRelation,selectMenu,renewData} from '@/api/system/menu'
    ...
    methods: {
        ...
        handleDelete() {//删除
            if(this.selectlistRow && this.selectlistRow.length==1){
                this.$confirm('此操作将删除所选中数据, 是否继续?', '提示', {
                  ...
                  deleteData(this.selectlistRow[0].catalogCode).then(response => {
                    this.getList()
                  })
                this.getTree()
                  this.$message({
        ...
        authority(){//授权
          ...
          if(this.selectlistRowUser && this.selectlistRowUser.length>0){
            ...
            addRelation(this.selectlistRow[0].catalogCode,endpointCodes,fun).then(response => {
              // this.getListUnelation()
                this.$message({
                    title: '成功',
                    message: '授权成功',
