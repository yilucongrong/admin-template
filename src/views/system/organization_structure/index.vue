        ...
        <div class="filter-container">
          <div class="main-header">
              <div class="select-content">
                <el-input size="small" :placeholder="$t('table.organizationNum')" v-model="listQuery.organizationCode" class="filter-item" @keyup.enter.native="handleFilter"/>
                <el-input size="small" :placeholder="$t('table.organizationName')" v-model="listQuery.organizationName" class="filter-item" @keyup.enter.native="handleFilter"/>
          ...
          <div class="main-body">
            ...
            <el-table
              ...
              <el-table-column show-overflow-tooltip :label="$t('table.organizationNum')" fixed prop="organizationCode" align="center" width="120">
                <template slot-scope="scope">
                  <span>{{ scope.row.organizationCode }}</span>
                </template>
              </el-table-column>
              <el-table-column show-overflow-tooltip :label="$t('table.organizationName')" fixed width="120px" align="center">
                <template slot-scope="scope">
                  <span>{{ scope.row.organizationName }}</span>
                </template>
              </el-table-column>
              <el-table-column show-overflow-tooltip :label="$t('table.organizationType')" fixed min-width="140px">
                <template slot-scope="scope">
                  <span>{{scope.row.typeCN}}</span>
                </template>
              </el-table-column>
              <el-table-column show-overflow-tooltip :label="$t('table.parentId')" width="120px" align="center">
                <template slot-scope="scope">
                  <span>{{ scope.row.parentId|organizationFilter(scope.row)}}</span>
                </template>
              </el-table-column>
              <el-table-column show-overflow-tooltip :label="$t('table.contacts')" align="center" width="120px">
                <template slot-scope="scope">
                  <span>{{ scope.row.contact }}</span>
                </template>
              </el-table-column>
              <el-table-column show-overflow-tooltip :label="$t('table.address')" width="120px" align="center">
                <template slot-scope="scope">
                  <span>{{ scope.row.adress }}</span>
                </template>
              </el-table-column>
              <el-table-column show-overflow-tooltip :label="$t('table.email')" width="150px">
                <template slot-scope="scope">
                  <span>{{ scope.row.email }}</span>
                </template>
              </el-table-column>
              <el-table-column show-overflow-tooltip :label="$t('table.businessFunction')" width="150px">
                <template slot-scope="scope">
                  <span>{{ scope.row.businessFunctionCN}}</span>
                </template>
              </el-table-column>
              <el-table-column show-overflow-tooltip :label="$t('table.mobile')" width="150px">
                <template slot-scope="scope">
                  <span>{{ scope.row.mobile }}</span>
                </template>
              </el-table-column>
              <el-table-column show-overflow-tooltip :label="$t('table.remarks')" width="150px">
                <template slot-scope="scope">
                  <span>{{ scope.row.remark }}</span>
                </template>
              </el-table-column>
              <el-table-column show-overflow-tooltip :label="$t('table.state')" width="80px">
                <template slot-scope="scope">
                  <span>{{ scope.row.state|stateFilter}}</span>
                </template>
              </el-table-column>
              <el-table-column show-overflow-tooltip :label="$t('table.postalCode')" class-name="status-col" width="120px">
                <template slot-scope="scope">
        ...
        <el-dialog custom-class="dialog-custom" :title="dialogStatus=='create'?$t('table.add'):$t('table.edit')" :visible.sync="dialogFormVisible" v-dialogDrag :close-on-click-modal="false">
          <el-form class="small-space" ref="dataForm" :inline="true" :rules="rules" :model="temp" label-width="120px">
              ...
              <el-form-item :label="$t('table.organizationNum')" prop="organizationCode" >
                <el-input v-if="dialogStatus=='update'" v-model="temp.organizationCode" disabled="disabled"/>
              ...
              <el-form-item :label="$t('table.organizationType')" prop="type">
                <el-select size="small" v-model="temp.type" :placeholder="$t('table.select')">
                  <el-option v-for="item in dt_org_type" :key="item.dictItemKey" :label="item.dictItemValue"
              ...
              <el-form-item :label="$t('table.businessFunction')" prop="businessFunction">
                <el-select size="small" v-model="temp.businessFunction" :placeholder="$t('table.select')">
                  <el-option v-for="item in dt_org_function" :key="item.dictItemKey" :label="item.dictItemValue"
                        :value="item.dictItemKey">
                  </el-option>
                </el-select>
              </el-form-item>
              ...
              <el-form-item :label="$t('table.address')">
                <el-input type="textarea" :rows="2" v-model="temp.adress"></el-input>
              </el-form-item>
...
import { selectlists,creat,selectlist,deletelist,updatalist,selecttree} from '@/api/system/organization'
  ...
  export default {
    ...
    methods:{
        ...
        getList() {//查询组织列表
            setTimeout(() => {
                selectlists(this.listQuery).then(response => {
                    ...
                    response.data = codeToName(response.data,options,['type','businessFunction']);
                    this.list = response.data.list
                    this.total = response.data.pages.count
                }),
        ...
        handleDownload() {
            ...
            const filterVal =  [
                'organizationCode', 'organizationName', 'typeCN',
                'parentId', 'contact','adress',
                'email','businessFunctionCN','mobile',
                'remark','state','postalCode'
                ]
